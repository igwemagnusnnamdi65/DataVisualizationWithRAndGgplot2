...
# Copyright 2018 Igwe Magnus Nnamdi

# This R source code file corresponds to the my project
# titled "An Introduction to Data Visualization with R and ggplot2" 
# github page https://github.com/igwemagnusnnamdi65/DataVisualizationWithRAndGgplot2
# I use titanic csv file in https://www.kaggle.com.
# You should know how to import file into RStudio
...
mytitanic <- read.csv("mytitanic.csv", stringsAsFactors = FALSE)
View(mytitanic)


# Set up factors.
mytitanic$Pclass <- as.factor(mytitanic$Pclass)
...
#
# I'll start our visual analysis of the data focusing on questions
...
#
ggplot(mytitanic, aes(x = Survived)) + 
  geom_bar()

# If you really want percentages.
prop.table(table(mytitanic$Survived))
...
#
ggplot(mytitanic, aes(x = Sex, fill = Survived)) + 
  theme_bw() +
  geom_bar() +
  labs(y = "Passenger Count",
...
#
ggplot(mytitanic, aes(x = Pclass, fill = Survived)) + 
  theme_bw() +
  geom_bar() +
  labs(y = "Passenger Count",
...
#
ggplot(mytitanic, aes(x = Age, fill = Survived)) +
  theme_bw() +
  geom_histogram(binwidth = 5) +
  labs(y = "Passenger Count",
       x = "Age (binwidth = 5)",
...
# plot.
ggplot(mytitanic, aes(x = Survived, y = Age)) +
  theme_bw() +
  geom_boxplot() +
  labs(y = "Age",
       x = "Survived",
...
ggplot(mytitanic, aes(x = Age, fill = Survived)) +
  ...
  labs(y = "Age",
       x = "Survived",
       title = "Titanic Survival Rates by Age, Pclass and Sex")
# This concludes my project on data visualization with R in RStudio.
# Please leave commits if found any mistakes in this project.
